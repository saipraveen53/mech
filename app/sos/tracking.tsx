...
if (Platform.OS === "web") {
  const {
    default: WebMapView,
    Marker: WebMarker,
  } = require("@teovilla/react-native-web-maps");
  MapView = WebMapView;
  Marker = WebMarker;
} else {
  const {
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (_, gesture) => {
        if (!isSlidingComplete && !isCompleting) {
          const newX = Math.max(
            0,
            Math.min(gesture.dx, sliderWidth - THUMB_SIZE),
          );
        ...
        if (
          ...
          setTimeout(() => {
            router.replace({
              pathname: "/payment",
              params: {
                mechanicName: mechanicName || "Emergency Mechanic",
                problem: problem || "Emergency Roadside Assistance",
                amount: "600", // Route params must be passed as strings
              },
            });
          }, 300);
        } else {
        ...
        <View style={styles.stepsContainer}>
          <View style={styles.step}>
            <View
              style={[
                styles.stepDot,
                status !== "searching" && styles.stepDotActive,
              ]}
            />
            <Text
              style={[
                styles.stepText,
                status !== "searching" && styles.stepTextActive,
              ]}
            >
              Finding
            </Text>
          ...
          <View style={styles.step}>
            <View
              style={[
                styles.stepDot,
                status === "arrived" && styles.stepDotActive,
              ]}
            />
            <Text
              style={[
                styles.stepText,
                status === "arrived" && styles.stepTextActive,
              ]}
            >
              Arrived
            </Text>
                ...
                style={[
                  styles.sliderProgress,
                  {
                    width: translateX.interpolate({
                      inputRange: [0, sliderWidth - THUMB_SIZE - 8], // Accounts for the thumb offsets
                      outputRange: [0, sliderWidth + 40], // Fills the complete track length
                      extrapolate: "clamp",
                    }),
                  },
                ]}
